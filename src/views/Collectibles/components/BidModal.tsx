...
const BidModal: React.FC<BidModalProps> = ({ nft, onSuccess, onFailed, bidify, onLowBalance, onDismiss }) => {
  ...
  return (
    <Modal title={t('Place a Bid')} onDismiss={onDismiss}>
      <ModalContent>
        <Label htmlFor="currentBid">{t(`Current Bid [`)}<TokenLogo src={logo} alt="token logo" /> {t(` ${symbol}]:`)}</Label>
        <StaticInput>{currentBid ? currentBid.toString() : 0}</StaticInput>
        <Label htmlFor="nextBid">{t(`Minimum Next Bid [`)}<TokenLogo src={logo} alt="token logo" /> {t(` ${symbol}]:`)}</Label>
        <StaticInput>{nextBid.toString()}</StaticInput>
        <Label htmlFor="yourBid">{t(`Your Bid [`)}<TokenLogo src={logo} alt="token logo" /> {t(` ${symbol}]:`)}</Label>
        <Input
          id="yourBid"
          name="yourBid"
          type="number"
