...
const Collectibles = (props) => {
  ...
  const getListsFromDatabase = async (): Promise<AxiosResponse> => {
    const ret = await axios.get(`${baseUrl}/auctions`, { params: { chainId } })
  ...
  useEffect(() => {
    ...
    const getCollections = async () => {
      ...
      if (_nfts.length === 0) {
        ...
        const _newNfts = newData.details.map((detail) => {
          return { ...detail, network: chainId }
        })
      ...
      <PageHeader>
        <StyledHero>
          ...
          <Heading as="h1" scale="lg" color="secondary">
            <a href="https://bidify.org" target="_blank" rel="noreferrer">
              {t('Powered by Bidify')}
            </a>
          </Heading>
        </StyledHero>
        <StyledToolbar>
          <StyledButton onClick={handleClickBrowse} variant="danger">
            Browse
          </StyledButton>
          <StyledButton onClick={handleClickSearch} variant="danger">
            Search
          </StyledButton>
          <StyledButton onClick={handleClickList} variant="danger">
            List an NFT
          </StyledButton>
          <StyledButton onClick={handleClickMint} variant="danger">
            Mint an NFT
          </StyledButton>
        </StyledToolbar>
        <StyledHead>
          ...
          {isAuction && searching && (
            <Flex alignItems="center">
              ...
              <IconButton
                ...
                scale="sm"
                variant="danger"
                onClick={() => !loading && setSearching(false)}
              >
      ...
      <Page>
        {loading ? (
          <CustomLoadingWrapper>
            ...
            {!isAuction && (
              <Heading mx={3} mt={3} maxWidth={600} as="h3">
                We are fetching your NFTs, due to the decentralised nature of Egem and IPFS, this may take a minute or
                two
              </Heading>
            )}
          </CustomLoadingWrapper>
